...
(main branch, allowed commit types, every package manager its lockfiles
name and the primary install command, check/verify/e2e scripts) rather than
assuming any of them, install the companion rule packs this repo's
...
one), more than one *Node* lockfile present (which package manager is
authoritative — lockfiles from different ecosystems, like `bun.lock` beside
`Cargo.lock`, are not a conflict and shouldn't produce a question).
...
for each), and whether any detected manager has no pack yet. Name the
lockfile behind each package-manager pack you installed, so a second one
reads as this repo's polyglot toolchain rather than a mistake. Tell me to
`/reload-plugins` if anything was installed — the rules aren't live in this
session until then. If `.omp/foreman.json` already exists, treat this as
