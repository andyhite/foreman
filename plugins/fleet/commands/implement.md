...
`skill://implement` drives `/tdd` at pre-agreed seams and closes with
`/code-review`. Both of those need decisions the worker cannot make alone, so
settle them here:
...
- **The seams to test at.** `/tdd` refuses to write a test at an unconfirmed
  seam, so a worker without them will either stall on a `fleet reply` or guess.
  Name the public interfaces under test.
...
## Seams to test at
<the public interfaces /tdd should work against>
