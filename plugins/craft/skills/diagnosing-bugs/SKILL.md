...
description: The diagnosis loop for hard bugs and performance regressions: build a red-capable feedback loop first, then reproduce, minimise, hypothesise, instrument, fix. Read when something is broken, throwing, failing, flaky, or slow.
---

# Diagnosing Bugs — the feedback loop is the skill
...
environment variables, so the credential stays in the environment rather
than in what you show. Captured artifacts carry auth headers: quote only the
...
consume it. If you don't have one, no amount of staring at code
will save you.
...
1. **Failing test** at whatever seam reaches the bug — unit, integration,
   e2e.
2. **Curl / HTTP script** against a running dev server. If the server is
   long-lived, start it with `hub` `op:"start"` rather than a bare shell
   command, so it stays up and loggable across turns.
...
6. **Throwaway harness.** Spin up a minimal subset of the system (one
   ...
   function call — as a long-lived process, run it via `hub` `op:"start"`
   instead of a bare shell command, so it survives across turns and its logs
   stay inspectable.
...
10. **HITL bash script.** Last resort. If a human must click, drive *them*
    with `hitl-loop.template.sh` so the loop is still structured. Captured
...
Stop and say so explicitly. List what you tried. Ask the operator for: (a)
access to whatever environment reproduces it, (b) a redacted captured
artifact (HAR file, log dump, core dump, screen recording with timestamps),
or (c) permission to add temporary production instrumentation. Do **not**
...
**already run at least once** (show the invocation and its output,
...
- [ ] **Agent-runnable** — you can run it unattended; a human in the loop
      only via `hitl-loop.template.sh`.
...
- The loop produces the failure mode the **operator** described — not a
...
moving parts left to suspect) and becomes the clean regression test in Phase
5.
...
**Show the ranked list to the operator before testing.** They often have
domain knowledge that re-ranks instantly ("we deployed a change to
#3"), or know hypotheses they've already ruled out. Cheap checkpoint, big
time saver. Don't block on it — proceed with your ranking if the operator is
AFK.
...
1. The `debug` tool (DAP-backed: breakpoints, stepping, variable inspection)
   if the target supports it. One breakpoint beats ten logs.
...
that finding belongs here, in the post-mortem, not a hand-off to another
pass. Name it precisely using the seam vocabulary in
`skill://codebase-design` (module, interface, depth, seam) and write it into
the commit / PR message. Make the recommendation **after** the fix is in,
