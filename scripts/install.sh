...
main() {
  ...
  case "$bun_version" in
    1.[0-2].*|0.*|"") die "bun 1.3+ is required (found ${bun_version:-none}) — https://bun.sh" ;;
  esac
  ...
  case "$FOREMAN_INSTALL_DIR" in
    *['"$`'\\]*) die "install dir contains a character that cannot be embedded in the wrapper: $FOREMAN_INSTALL_DIR" ;;
  esac
