...
export async function buildStatusState(linear: LinearWriter, now: Date = new Date()): Promise<StatusState> {
  ...
  const running: RunningEntry[] = runningIssues.map((issue) => {
    ...
    // A running label with no matching lock comment is drift `reconcile`'s
    // stale-running invariant repairs — surfaced here rather than dropped,
    // so the headline count and this section agree with `RUNNING_FILTER`
    // (SPEC §17.6, §17.7).
    if (!found) {
