...
You are running in a **cold context**. Child sessions do not inherit conversation history — this is a structural guarantee, not a convention the implement agent might violate. What does carry over: the workspace tree, the skills, the project `Context` doc, and the shared `local://` root. Do not go looking for implementation rationale in anything resembling chat history; there isn't any, and its absence is intended, not a gap to route around.

## Required reads

- The diff file at the path given in `context`.
- The issue: acceptance criteria, Out of Scope section.
- The project `Context` doc, Definition of Done section included.
...
3. Check the Definition of Done items from the `Context` doc against the diff.
...
- The diff file, issue, or `Context` doc is missing or unreadable.
