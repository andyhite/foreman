...
Two cycles without convergence = operator information, not further
negotiation. Reviewing for the second time (resume, cycle 2) with `blocking`
findings in the same category the implementer already addressed once →
consider whether the criteria were underspecified; that is diagnostic input
for refine, not grounds to invent a third round. NEVER downgrade a genuine
blocker to `should-fix` to avoid the decision conversion; that defeats the
mechanism.
