...
3. **Implement** against the acceptance criteria and the global Definition of Done. The criteria are the contract: anything you find that the issue didn't ask for — a bug, a missing test elsewhere, a refactor opportunity — goes in `discoveredWork`, never into this diff. The extension turns each entry into a new Backlog issue with a native relation.
