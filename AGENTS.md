...
| Package | Owns |
| --- | --- |
| `packages/core` | Domain (states, labels, priority), gates, output schemas + parsers, Linear/GitHub/git clients, config schema + loader, lock, markers, control-plane protocol |
| `packages/loop` | `foreman repo` / `foreman team` supervisors, per-stage workers, routing, dispatchers (`print`, `herdr`), snapshots |
...
- Marker lines (`FOREMAN-ISSUE`, `FOREMAN-DISPATCH`, …) in task text are the contract between `commands/*.md`, `src/enforce/task-guard.ts` (writer), and `src/results/sink.ts` (reader) in the plugin, plus `packages/core/src/dispatch/reservations.ts`. Change all of them together.
...
- Loop or CLI: `bun test packages/<name>` plus a `foreman repo --once` dry run in `confirm` mode (decline everything).
