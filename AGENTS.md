...
`plugins/fleet/` is a harness-portable agent plugin (omp, Claude Code, Codex,
Cursor) whose commands and skills teach an orchestrator to drive that CLI.
`.claude-plugin/` and `.omp-plugin/` at the root are the marketplace manifests
that publish the same plugin tree under the name `foreman`.
...
- Worker-facing prose names skills through `fleet skill <name>` — the one form
  every harness resolves. The sweep test in `fleet-test.sh` fails any
  `skill://` URI found in plugin prose or the root README.
- One version string, seven files: `herdr/herdr-plugin.toml`, the four
  `plugins/fleet/.*-plugin/plugin.json`, and both root marketplace manifests
  must agree (CI enforces). `fleet version` reads the toml at runtime.
- Two names, deliberately: `foreman` is the marketplace (named for this
  ...
  is `fleet@foreman` in every harness. Do not introduce a third name, and do
...
- `plugins/fleet/` prose stays harness-neutral: no tool names, URIs, or env
  vars that only one harness understands.
